...
C     ----------------
C     jw / 25-07-12  1st draft started
C     jw / 20-10-14  last rev.

C     In any un-saved data then prompt to call FSave or FSaveAS if reqd.


C     CALL ClearAllData
C     IF(.NOT.F%NAMED) CALL GetFileName
...
C     If required, call the function CHANGED() to obtain a new ChkSUM
...
10    IF(.NOT.F%NAMED) CALL GetFileName(.TRUE.)
...
C        IF(INDEX(LINE,STRING(1:Len_Trim(String))).NE.0) THEN
         IF(INDEX(LINE,STRING(1:LenStr)).NE.0) THEN
...
C     the same filename, and prompt for a different name if it is
     ...
     +            'Rename your old one before continuing?',YN)
