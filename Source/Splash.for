...
C     ------------------------------
      ...
      CALL HEIGHT(LH)
C     ------------------------------
        CALL MESSAG('Program '//PROG8,LX,LY)       ! Program INSTRUCT
...
C     CALL COMPLX
      ...
      CALL HEIGHT(LH)
C     ------------------------------
      CALL MESSAG(DESC48,LX,LY)    ! Structural analysis of plane frames
...
C     CALL WINFNT('Times New Roman')
      ...
      CALL HEIGHT(LH)
C     ------------------------------
      CALL MESSAG(TRIM(DESC70)// ! Single self-contained program file.
     +                           ! Nothing to install.
     +  ' ('//TRIM(I2CHAR(Pan3W,idum))//'x'//
     +  TRIM(I2CHAR(Pan3H,idum))//')', LX,LY)
      LY = LY+NINT(FLOAT(LH)*OVS)+LS

      CALL MESSAG('Version '//VERS4//' ('//PDAT9//')' ! Pla.fra.analysis
     +  //'  by '//TRIM(AUTH16)//', '//TRIM(FRM32), LX,LY) ! by Jmw
      LY = LY+NINT(FLOAT(LH)*OVS+2.4*FLOAT(LS))
...
C     ------------------------------
      CALL MESSAG
     +  ('THE HELP-MENU HAS CONTEXT-RELATED GUIDANCE '//
     +   'FOR EACH STAGE OF THE PROGRAM.', LX,LY)
	  LY = LY+NINT(FLOAT(LH)*OVS)+3*LS
...
C     ------------------------------
     ...
     +  'program author.',LX,LY)
      ...
      CALL MESSAG(EML24,LX,LY)                    ! eMail address
	  LY = LY+NINT(FLOAT(LH)*OVS)+LS

      CALL MESSAG(TEL1,LX,LY)                     ! Telephone-1
	  LY = LY+NINT(FLOAT(LH)*OVS)+LS

      CALL MESSAG(TEL2,LX,LY)                     ! Telephone-2
	  LY = LY+NINT(FLOAT(LH)*OVS)+3*LS
